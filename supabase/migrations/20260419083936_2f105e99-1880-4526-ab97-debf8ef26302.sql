...
create policy "Resumes update own" on public.resumes for update using (auth.uid() = user_id);
...
create policy "SavedJobs all own" on public.saved_jobs for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
...
create policy "Interview all own" on public.interview_sessions for all using (auth.uid() = user_id) with check (auth.uid() = user_id);